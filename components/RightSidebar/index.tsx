...
const RightSidebar = ({ className, visible }: RightSidebarProps) => {
    ...
    const fetchChatHistory = async () => {
        ...
        try {
            const response = await axios.get("https://medibot-backend-docker.redcliff-fa17bb4c.uaenorth.azurecontainerapps.io/chat/chats", {
                headers: {
                    '_id': _id,
                    'token': token,
                },
            ...
            >
                ...
                <div className="grow overflow-y-auto scroll-smooth px-6 md:px-3">
                    {clean ? (
                        ...
                        chatHistory.slice().reverse().map((x) => <ChatItem item={x} key={x._id} />)
                    )}
                </div>
